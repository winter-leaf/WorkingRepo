...
1. 按照address -> data -> response channel的顺序察看
2. 在address channel中察看ID，LOCK和BURST相关信号来确定当前发布的指令是什么
3. 等待address channel中的握手
4. 对于data channel，察看WSTRB(如果是write data channel)，察看LAST信号确定当前信息的结束点
5. 等待data channel中的握手
6. 对于response channel，察看ID((若是写操作))，察看data channel的LAST信号，察看BRESP信号
7. 等待等待response channel中的握手
